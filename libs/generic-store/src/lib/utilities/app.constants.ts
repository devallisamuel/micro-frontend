...
import {
  ...
  SetIdConfiguration,
} from '../_store/app-detail/app.reducer';
...
export class Configuration {
  ...
  public getConstantJson(): Observable<Constants> {
    return this._http
      .get(this.constantsFile)
      .pipe(map((res: Constants) => res));
  }
  public load() {
    return new Promise((resolve, reject) =>
      this.getConstantJson().subscribe((c) => {
  ...
  //         this.store.dispatch(
  //           new SetIdConfiguration(this.authWellKnownEndpoints)
  //         );
